...
| `DRIVERS_TYPE`                    | `Shared`| How to build and link against `TinyDrivers` SQL database drivers.<br/><small>The `Static` value will be select by default when the `BUILD_SHARED_LIBS` is `OFF`.<br/>Supported values: `Shared`, `Loadable`, and `Static`<br/>Available when: `BUILD_DRIVERS AND BUILD_SHARED_LIBS`</small> |
