...
Installing the `vcpkg` is highly recommended, it simplifies installation of `range-v3` dependency.
...
| `TINYTOM_MIGRATIONS_PATH`         | Default migrations path for the `make:migration` command.<br/><small>Default value: `/examples/tom/database/migrations`</small><br/><small>Defined by [`TOM_MIGRATIONS_PATH`](#TOM_MIGRATIONS_PATH) <small>(cmake)</small> configuration build option.<br/><small>(qmake note) You can use `DEFINES += TINYTOM_MIGRATIONS_PATH="\"database/migrations\""` on the command-line or set it in the __main__ [`conf.pri`](https://github.com/silverqx/TinyORM/blob/develop/conf.pri.example#L4-L9) file.</small></small> |
...
| `TOM_MIGRATIONS_PATH`             | `-`     | Default migrations path for the `make:migration` command.<br/><small>Default value: `/examples/tom/database/migrations`</small> |
...
Now you can try the [`HelloWorld CMake`](hello-world.mdx#hello-world-with-cmake) example.
...
<img src={require('./assets/img/building/qmake-configure_project.png').default}
...
<img src={require('./assets/img/building/qmake-build_settings.png').default}
...
<img src={require('./assets/img/building/qmake-additional_arguments.png').default}
