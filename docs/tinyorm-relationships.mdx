...
The rule of thumbs are:
 ...
 - you have to always pass the `Related` parameter in nested relations, you can not use nested relations with `Related = void`
...
You are able to use any of the Laravel [query builder's](query-builder.mdx) methods on the relationship, so be sure to explore the query builder documentation to learn about all of the methods that are available to you.
