...
:::info
The PostgreSQL's libpq client library provides the [`PGSSLMODE`](https://www.postgresql.org/docs/current/libpq-envars.html#id-1.7.3.22.3.4.13.1.1) environment variable, so don't have to use TinyORM's `options` configuration and use the `PGSSLMODE` instead.
:::
