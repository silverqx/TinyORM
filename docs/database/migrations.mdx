...
You can enable tab completion using the following command. The `tom.exe` and `TinyOrm0.dll` library must be on the system path for the tab completion to work properly. Currently is available only for the `pwsh` shell.

```powershell
tom integrate pwsh
```

Or you can paste the tab completion code to the pwsh profile manually.
...
It provides completion for the commands, long and short parameters, and also for some positional arguments like namespaces for the list command or commands for the help command.
