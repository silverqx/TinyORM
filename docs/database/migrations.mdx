...
Tab completion is currently available for the `pwsh` (on Linux too) and `bash` shells. For `pwsh` the `tom.exe` and `TinyOrm0.dll` library must be on the system path to work properly. With `bash` if the `tom` executable and `libTinyOrm.so` library is not on the system paths then it will provide less accurate completions.
...
Or you can enable the tab completion manually. Following actions are the same as the `tom integrate` command does.
