                                                    ...
                                                    const ulong valuesSize)
{
    ...
    if (valuesSize > placeholdersCount)
        qWarning().noquote()
                << sl("MySqlResultPrivate::hasPreparedBindings: "
    ...
    if (placeholdersCount > valuesSize)
        qWarning().noquote()
                << sl("MySqlResultPrivate::hasPreparedBindings: "
                      ...
                      "placeholder markers is '%1' and number of bind values is '%2', "
                      "skipping prepared bindings processing")
                   .arg(placeholdersCount)
                   .arg(valuesSize);
}
        ...
        const uint fieldsCount, const uint lastIndex, const QLatin1StringView method)
{
    ...
    qWarning().noquote()
            << sl("MySqlResultPrivate::%1: fieldsCount != index, column "
...
QVariant MySqlResultPrivate::toDoubleFromString(QString &&value) const
{
    ...
    if (!ok) {
        qWarning().noquote()
            << sl("MySqlResultPrivate::toQVariantDouble: unable to convert QString "
                  "to the double type, QString value is '%1'").arg(value);
        return {};
    }
