        ...
        const QString &model, const QString &relation, const From from)
{
    ...
    /* I was confused when I looked at this method after some time, but it's correct,
       ...
       argument (UNDEFINED). */
    static const auto BelongsToMessageTmpl =
            u"Can not guess the relationship name for '%2' relation type, "
             ...
             "(eg. QString::fromUtf8(__func__))."_s;

    switch (from) {
    case From::BELONGS_TO:
        return BelongsToMessageTmpl.arg(model, u"belongsTo"_s, relation);

    case From::BELONGS_TO_MANY:
        return BelongsToMessageTmpl.arg(model, u"belongsToMany"_s, relation);

    default:
        return u"The relationship mapping '%1' doesn't exist in the 'u_relations' map "
